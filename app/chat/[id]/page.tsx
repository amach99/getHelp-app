...
export default function ChatPage() {
  ...
  // Load messages (stored under tickets/{id}/messages)
  useEffect(() => {
    if (!ticketId) return;
    const q = query(
  ...
  async function handleSend(e: React.FormEvent) {
    e.preventDefault();
    if (!text.trim() || !user || !profile || !ticket) return;
    setSending(true);
    try {
  ...
  if (!ticket) {
    return (
      <div className="max-w-xl mx-auto px-4 pt-28 text-center">
        <p className="text-muted">This ticket no longer exists.</p>
  ...
  if (!hasAccess) {
    return (
      <div className="max-w-xl mx-auto px-4 pt-28 text-center">
        ...
        <p className="text-muted mb-6">You need to accept this request to chat.</p>
        <Link href={`/tickets/${ticketId}`} className="btn-primary">View Ticket</Link>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-screen pt-16">
      ...
      <div className="glass border-b border-white/20 px-4 py-3 flex items-center gap-3 z-10">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-display font-bold text-slate-800 truncate leading-tight">{ticket.title}</h2>
          <p className="text-xs text-muted flex items-center gap-1">
            <span className={`category-badge ${style.bg} ${style.text} py-0 px-1.5`}>{ticket.category}</span>
            · {ticket.location.address}
          </p>
        </div>
        <Link href={`/tickets/${ticketId}`} className="p-2 text-muted hover:text-primary transition-colors">
          <Info size={16} />
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50 px-4 py-4 space-y-3">
        ...
        <div className="text-center">
          <span className="text-xs bg-slate-200 text-slate-500 px-3 py-1 rounded-full font-medium">
            Chat started · {ticket.requesterName} posted this request
          </span>
        </div>
        ...
        {messages.map((msg, i) => {
          ...
          const showDate =
            i === 0 ||
            format(messages[i - 1].createdAt?.toDate?.() ?? new Date(), "yyyy-MM-dd") !== format(ts, "yyyy-MM-dd");
      ...
      {ticket.status !== "completed" && ticket.status !== "cancelled" ? (
        ...
        <div className="bg-slate-50 border-t border-slate-100 px-4 py-3 text-center text-sm text-muted font-medium">
          This request is {ticket.status}. Chat is now read-only.
        </div>
      )}
    </div>
  );
